...
const Babyfetch = (props) => {
  ...
  useEffect(() => {
    axios
      ...
      .then((res) => {
        ...
        const filterdata = res.data.filter((item) => item.role === "baby");
        setData(filterdata);
      })
      .catch((error) => {
        console.log(error);
  ...
  let role = props.product;
  useEffect(() => {
    const filteredMen = role.filter((item) => item.role === "baby");
    setBaby(filteredMen);
  }, [props.product]);
        ...
        axios
          ...
          .then((res) => {
            ...
            if (result.status === "success") {
              ...
              // Update the 'liked' property for the specific product
              setBaby((prevBaby) =>
            ...
            <tbody>
              {baby.map((item, index) => (
                <tr key={index}>
                  ...
                  <td>{item.name}</td>
                  <td><strike>{item.strike}</strike></td>
                <td>{item.price}</td>
